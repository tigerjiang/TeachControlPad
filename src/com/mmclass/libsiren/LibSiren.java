...
public class LibSiren {
	...
	static {
		...
		try {
			...
			Log.e(TAG,
					"Encountered a security issue when loading sirenjni library: "
							+ se);
			// / FIXME Alert user
			System.exit(1);
		}
	...
	public int openHeadset() {
		return headsetSet(true);
	}

	public int closeHeadset() {
		return headsetSet(false);
	}
	...
	public int openLinein() {

		return lineinSet(true);
	}

	public int closeLinein() {

		return lineinSet(false);
	}
    
        public int setToneUp(int value) {
		return toneUpSet(value);
	}
	...
	private native void sendMessageToGroupNative(String message, String ip,
			String port);
	...
	// 1 -- close
	// other -- error
	private native int headsetGet();

	private native int headsetSet(boolean flag);

	// 0 -- open
	// 1 -- close
	// other -- error
	private native int lineinGet();

	private native int lineinSet(boolean flag);
  
        private native int toneUpSet(int value);  
